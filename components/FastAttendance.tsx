...
export const FastAttendance: React.FC<FastAttendanceProps> = ({ students, onClose, onSave, date }) => {
    ...
    return (
        <div className="fixed inset-0 bg-slate-900 z-[60] flex flex-col justify-center items-center overflow-hidden" aria-modal="true" role="dialog">
            ...
            <button onClick={handlePartialClose} className="absolute top-4 right-4 p-2 bg-black/20 hover:bg-black/40 rounded-full text-white/70 hover:text-white z-50 backdrop-blur-md transition-colors"><XIcon className="w-6 h-6" /></button>

            {isFinished ? (() => {
                const presentIds = new Set(records.filter(r => r.status === AttendanceStatus.PRESENT || r.status === AttendanceStatus.LATE).map(r => r.studentId));
                const totalPresent = presentIds.size;
                const female = students.filter(s => presentIds.has(s.id) && s.gender === 'F').length;
                const male = students.filter(s => presentIds.has(s.id) && s.gender === 'M').length;

                return (
                    <div className="text-center text-white z-10 animate-fade-in-up w-full max-w-sm px-4">
                        ...
                        <h2 className="text-3xl font-black mb-1">¡Lista completa!</h2>
                        <p className="mb-6 text-slate-300 text-sm">{students.length} estudiantes marcados</p>

                        {/* Stats */}
                        <div className="space-y-3 mb-6">
                            <div className="bg-white/10 backdrop-blur-md rounded-2xl p-4 border border-white/10">
                                <p className="text-3xl font-black text-blue-300">{totalPresent}</p>
                                <p className="text-xs font-bold uppercase tracking-widest text-blue-400 mt-1">Total Presentes</p>
                            </div>
                            <div className="grid grid-cols-2 gap-3">
                                <div className="bg-white/10 backdrop-blur-md rounded-2xl p-3 border border-white/10">
                                    <p className="text-2xl font-black text-pink-300">{female}</p>
                                    <p className="text-xs font-bold uppercase tracking-widest text-pink-400 mt-1">Femenino</p>
                                </div>
                                <div className="bg-white/10 backdrop-blur-md rounded-2xl p-3 border border-white/10">
                                    <p className="text-2xl font-black text-sky-300">{male}</p>
                                    <p className="text-xs font-bold uppercase tracking-widest text-sky-400 mt-1">Masculino</p>
                                </div>
                            </div>
                        </div>
                ...
                <>
                                    ...
                                    <div className="pointer-events-none select-none z-0 p-6 w-full h-full flex flex-col items-center justify-center">
                                        <div className="flex flex-col items-center justify-center gap-2 mb-2">
                                            <div className="relative w-28 h-28">
                                                <div className="w-full h-full rounded-full border-4 border-slate-100 shadow-sm overflow-hidden">
                                                    <Avatar
                                                        src={student.avatar}
                                                        name={student.name}
                                                        size="xl"
                                            ...
                                            {student.orderNumber && (
                                                <span className="text-5xl font-black text-slate-800 tracking-tighter mt-1">
                                                    {student.orderNumber}
                                                </span>
                                            )}
                                        </div>
                                        <h3 className="text-xl font-extrabold text-slate-800 mb-1 line-clamp-2 leading-tight min-h-[3rem] flex items-center justify-center">{student.name}</h3>
                                    </div>
                                </div>
                            );
                        })}
                    ...
                    <div className="mt-10 flex flex-col items-center gap-6 z-10">
                        <div className="flex items-center justify-center gap-6 text-white">
                            ...
                            <button onClick={() => handleSwipe(AttendanceStatus.PRESENT)} disabled={isAnimating} title="Presente (Derecha)" className="bg-green-500 p-4 rounded-full shadow-lg shadow-green-500/30 hover:bg-green-400 hover:scale-110 transition-all active:scale-95 disabled:opacity-50"><CheckIcon className="w-6 h-6" /></button>
                        </div>
