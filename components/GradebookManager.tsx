...
import { PlusIcon, AcademicCapIcon, BoltIcon, PencilIcon, DocumentTextIcon, ChevronDownIcon, TrophyIcon, UserGroupIcon, TrashIcon } from './icons';
...
const TabButton: React.FC<{ label: string; icon: React.ReactNode; isActive: boolean; onClick: () => void; className?: string }> = ({ label, icon, isActive, onClick, className = '' }) => (
    <button
        onClick={onClick}
        className={`flex items-center justify-center gap-2 px-4 py-3 text-sm font-semibold rounded-t-lg border-b-2 transition-colors focus:outline-none ${
            isActive
  ...
  for (const student of classStudents) {
    ...
    for (const group of competencyGroups) {
      ...
      for (const period of evaluationPeriods) {
        ...
        const finalPeriodScore = recoveryGrade && (periodScore === null || recoveryGrade.score > periodScore)
          ? recoveryGrade.score
          : periodScore;
...
const calculatePeriodDetails = (
    ...
    return classStudents.map(student => {
        ...
        const periodAverage = totalPossible > 0 ? Math.round((totalScored / totalPossible) * 100) : null;
        ...
        for (const group of competencyGroups) {
            ...
            pcScores[group] = periodGroupScore;
        }

        return { student, instrumentGrades, periodAverage, pcScores, recoveryPeriodGrades };
    });
};
...
export const GradebookManager: React.FC<GradebookManagerProps> = ({ students, classes, fundamentalCompetencies, competencies, instruments, grades, recoveryGrades, onAddCompetencyClick, onAddInstrumentClick, onEditInstrumentClick, onViewInstrumentDetails, initialTab, onExpressGradingClick, onAddRecoveryGradeClick, studentFilter, onClearStudentFilter, initialFundamentalFilter, selectedClassId, onSelectClass, onAddStudentClick }) => {
  ...
  const gradeSheetData = useMemo(() => {
    ...
    const data = calculateGradeSheet(classStudents, classInstruments, grades, recoveryGrades, competenciesByGroup);
    return { data, classStudents };
  }, [selectedClassId, students, studentFilter, instruments, grades, recoveryGrades, competenciesByGroup]);
  ...
  const periodDetailData = useMemo(() => {
    ...
    const studentPeriodGrades = calculatePeriodDetails(periodInstruments, classStudents, grades, recoveryGrades, competenciesByGroup, selectedPeriod);
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6 gap-4">
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 w-full lg:w-auto">
                ...
                <div className="flex items-center p-1 bg-slate-100 dark:bg-slate-700/60 rounded-lg w-full sm:w-auto">
                    <button onClick={() => {setGradeViewType('summary'); setExpandedStudentId(null);}} className={`flex-1 sm:flex-none px-6 py-2 text-sm font-bold rounded-md transition-all duration-200 ${gradeViewType === 'summary' ? 'bg-white dark:bg-slate-800 shadow-sm text-indigo-600 dark:text-indigo-400' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'}`}>
                        Resumen Anual
                    </button>
                    <button onClick={() => {setGradeViewType('period'); setExpandedStudentId(null);}} className={`flex-1 sm:flex-none px-6 py-2 text-sm font-bold rounded-md transition-all duration-200 ${gradeViewType === 'period' ? 'bg-white dark:bg-slate-800 shadow-sm text-indigo-600 dark:text-indigo-400' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'}`}>
                        Detalle de Período
                    </button>
                </div>

                 {/* Period View Specific Controls */}
            ...
            <div className="flex flex-wrap items-center gap-4 w-full lg:w-auto lg:justify-end">
                {gradeViewType === 'period' && (
                 ...
                 <div className="flex items-center gap-2">
                    <label htmlFor="show-recovery-toggle" className="text-sm font-medium text-slate-600 dark:text-slate-300 cursor-pointer select-none">Recuperación (RP)</label>
                    <button
            ...
            <div className="overflow-x-auto">
                {periodDetailData.periodInstruments.length > 0 ? (
                    <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-700 border-collapse text-sm">
                        ...
                        <tbody className="bg-white dark:bg-slate-800 divide-y divide-slate-200 dark:divide-slate-700">
                            {periodDetailData.studentPeriodGrades.map(({ student, instrumentGrades, periodAverage, pcScores, recoveryPeriodGrades }, idx) => (
                                <tr key={student.id} className={idx % 2 === 0 ? 'bg-white dark:bg-slate-800' : 'bg-slate-50/50 dark:bg-slate-800/50'}>
                                    <td className="px-4 py-3 whitespace-nowrap font-medium text-slate-900 dark:text-slate-100 sticky left-0 z-10 bg-inherit border-r border-slate-200 dark:border-slate-700 shadow-[4px_0_24px_-2px_rgba(0,0,0,0.05)]">
                                        <div className="flex items-center gap-3">
                                            <div className="w-8 h-8 rounded-full bg-slate-200 dark:bg-slate-700 overflow-hidden flex-shrink-0">
                                    ...
                                    {competencyGroups.flatMap(group => [
                                        <td key={`pc-cell-${student.id}-${group}`} className="px-1 py-2 whitespace-nowrap text-center border-l border-slate-200 dark:border-slate-700 bg-slate-50/30 dark:bg-slate-800/30">
                                            <span className={`text-xs font-bold ${pcScores?.[group] !== null && pcScores?.[group]! < 70 ? 'text-red-600 dark:text-red-400' : 'text-slate-700 dark:text-slate-300'}`}>
                                                {pcScores?.[group] ?? '-'}
                                            </span>
                                        </td>,
                                        showRecoveryGrades && (
                                            <td key={`rp-cell-${student.id}-${group}`} className="px-1 py-2 whitespace-nowrap text-center border-l border-slate-200 dark:border-slate-700 bg-blue-50/10 dark:bg-blue-900/10">
                                                <button onClick={() => onAddRecoveryGradeClick(student, selectedPeriod, group, pcScores?.[group] ?? null)} className={`w-8 h-6 text-xs rounded hover:bg-blue-100 dark:hover:bg-blue-800 transition-colors ${recoveryPeriodGrades?.[group] !== null && recoveryPeriodGrades?.[group] !== undefined ? 'text-blue-700 dark:text-blue-300 font-bold' : 'text-slate-400'}`}>
