...
const TabButton: React.FC<{ label: string; icon: React.ReactNode; isActive: boolean; onClick: () => void; }> = ({ label, icon, isActive, onClick }) => (
    <button
        onClick={onClick}
        className={`flex items-center justify-center w-full px-4 py-3 font-semibold text-sm rounded-lg transition-colors duration-200 ${
            isActive
...
export const TeacherProfile: React.FC<TeacherProfileProps> = ({ profile, classes, students, journalEntries, resources, onAddJournalEntry, onAddResource, onClassClick, onLogout, onUpdateProfile }) => {
  ...
  const handlePhotoChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
        ...
        alert("Error al actualizar la foto de perfil. Por favor, intente nuevamente.");
    } finally {
