...
export const ExpressGradingModal: React.FC<ExpressGradingModalProps> = ({ isOpen, onClose, instrument, students, grades, onSaveGrades, initialFocusStudentId }) => {
  ...
  const handleCriteriaChange = (studentId: string, criterionId: string, value: boolean | number) => {
    setLocalGrades(prev => {
        const updatedStudentGrades = { ...prev[studentId] };
        updatedStudentGrades.criteria = { ...updatedStudentGrades.criteria, [criterionId]: value };
        return { ...prev, [studentId]: updatedStudentGrades };
    });
  }
          ...
          <tr key={student.id} className="border-t border-slate-200 dark:border-slate-700">
            ...
            {instrument?.criteria?.map(c => (
              <td key={c.id} className="p-2 text-center">
                ...
                {instrument.type === 'Escala Estimativa' && (
                   <select value={(localGrades[student.id]?.criteria[c.id] as number) ?? 0} onChange={e => handleCriteriaChange(student.id, c.id, parseInt(e.target.value))}
                    className="w-full text-center border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-white rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm p-1">
