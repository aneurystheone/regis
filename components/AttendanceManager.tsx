      ...
      <div className="p-4 sm:p-8 space-y-6">
              ...
              {schoolMonths.map(month => {
                ...
                return (
                  <button
                    ...
                    className={`flex-shrink-0 flex flex-col items-center justify-center p-3 rounded-lg transition-colors w-28 h-24 ${
                      isSelected ? 'bg-indigo-600 text-white shadow-md' : 'bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-700 border dark:border-slate-700'
                    }`}>
                ...
                {daysInSelectedMonth.map(day => {
                  ...
                  return (
                    <button key={day} ref={isSelected ? activeDayRef : null} onClick={() => setSelectedDate(dayDateString)} title={capitalizedWeekday}
                      className={`flex-shrink-0 flex items-center justify-center font-semibold transition-colors text-sm h-10 ${
                        isSelected ? 'w-10 rounded-full bg-slate-800 text-white shadow-md dark:bg-slate-200 dark:text-slate-800'
        ...
        {!isWeekend && (
          <>
              ...
              <div className="fixed bottom-8 right-8 z-30 flex flex-col items-end pointer-events-none">
