...
export const AddCompetencyModal: React.FC<AddCompetencyModalProps> = ({ isOpen, onClose, onAddCompetencies, classes, competencies, selectedClassId }) => {
  ...
  useEffect(() => {
    const fetchCurriculum = async () => {
        ...
        if (isOpen && currentClass) {
            ...
            // Reset state on close or if no class is selected
            setMode('import');
            setSearchQuery('');
            setSelectedCodes([]);
            setCustomName('');
  ...
  const handleCustomSubmit = (e: React.FormEvent) => {
      ...
      const newCompetency: Omit<Competency, 'id'> = {
          classId: currentClass.id,
          fundamentalId: 'FC3', // Defaulting to a generic one as UI doesn't support selecting it yet
          name: customName,
          code: customCode,
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-2xl transform transition-all flex flex-col max-h-[90vh]" role="document">
        <div className="flex-shrink-0 p-6 border-b border-slate-200 dark:border-slate-700">
            <div className="flex justify-between items-start">
                <div>
                    <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-100">Añadir Competencia Específica</h2>
